...
void initLock() {
  ...
  CTRL_LOG_PRINTF("[REED] boot raw=%d interpreted=%s (REED_CLOSED_IS_HIGH=%d)\n",
...
LockStatus tryUnlock(bool ignoreReed) {
  ...
  if (coilTempC >= LOCK_THERMAL_MAX_TEMP) {
    CTRL_LOG_PRINTLN(F("[LOCK] Blocked — coil overheated"));
  ...
  if (ignoreReed) {
    CTRL_LOG_PRINTLN(F("[LOCK] Unlocked forcefully (admin override, reed ignored, holding)"));
  } else {
    CTRL_LOG_PRINTLN(F("[LOCK] Unlocked, holding for user to lift lid (10s max)"));
  }
...
LockStatus tryLock(bool ignoreReed) {
  ...
  if (ignoreReed) {
    reedStable = true;
    CTRL_LOG_PRINTLN(F("[LOCK] Locked forcefully (admin override, reed ignored)"));
    return LOCK_OK;
  }
  ...
  if (!closed) {
    CTRL_LOG_PRINTLN(F("[LOCK] STUCK OPEN — reed still reads open"));
    return LOCK_STUCK_OPEN;
  }

  reedStable = true;
  CTRL_LOG_PRINTLN(F("[LOCK] Locked OK"));
  return LOCK_OK;
}
...
bool maintainLockSafety(unsigned long now) {
  ...
  if (!prevQualifiedOpen && qualifiedOpen && !cooldownActive) {
    tamperLatch = true;
    tamperLastAt = now;
    CTRL_LOG_PRINTLN(F("[TAMPER] Reed opened without unlock — TAMPER DETECTED"));
  }
  ...
  if (unjamDeEnergiseAt > 0 && now >= unjamDeEnergiseAt) {
    deEnergise();
    unjamDeEnergiseAt = 0;
    CTRL_LOG_PRINTLN(F("[UNJAM] Pulse de-energised (300ms elapsed)"));
  }
  ...
  if (solenoidOnAt > 0 && (now - solenoidOnAt >= LOCK_MAX_ACTIVE_MS)) {
    ...
    CTRL_LOG_PRINTLN(F("[LOCK] THERMAL CUTOFF (timeout limit)"));
    return true;
  }
...
void initSelfTest(bool skipSolenoid) {
  ...
  CTRL_LOG_PRINTF("[SELFTEST] Init — baseline reed=%s, solenoid=%s\n",
...
SelfTestResult tickSelfTest(unsigned long now) {
  ...
  if (stStage == 0) {
    ...
    if (stSkipSolenoid) {
      ...
      CTRL_LOG_PRINTLN(F("[SELFTEST] Reed-only mode — solenoid pulses SKIPPED (active delivery)"));
    ...
    energise(now);
    stStage = 1;
    CTRL_LOG_PRINTF("[SELFTEST] Pulse %u/%u — energise\n",
  ...
  if (stStage % 2 == 1) {
    if (now - stStageStartAt >= SELFTEST_PULSE_DURATION_MS) {
      deEnergise();
      stPulsesDone++;
      CTRL_LOG_PRINTF("[SELFTEST] Pulse %u/%u — de-energise\n",
  ...
  if (stStage < 254) {
    if (now - stStageStartAt >= SELFTEST_INTERVAL_MS) {
      ...
      if (currentReed != stBaselineReed) {
        CTRL_LOG_PRINTF("[SELFTEST] Reed drift during settle: was=%d now=%d\n",
  ...
  if (stStage == 254) {
    ...
    if (now - stStageStartAt >= settleTime) {
      ...
      if (finalReed != stBaselineReed) {
        CTRL_LOG_PRINTF("[SELFTEST] Reed mismatch after test: baseline=%d final=%d\n",
                      stBaselineReed, finalReed);
        stReedFault = true;
      }

      if (stReedFault) {
        CTRL_LOG_PRINTLN(F("[SELFTEST] WARN — reed was unstable during test"));
        return SELFTEST_WARN;
      }

      CTRL_LOG_PRINTLN(F("[SELFTEST] PASS — solenoid and reed nominal"));
...
UnjamResult fireUnjamPulse(unsigned long now) {
  ...
  // Guard: thermal
  if (coilTempC >= LOCK_THERMAL_MAX_TEMP) {
    CTRL_LOG_PRINTLN(F("[UNJAM] Blocked — coil overheated"));
    return UNJAM_OVERHEATED;
  }

  // Guard: cooldown
  if (unjamLastPulseAt > 0 && now - unjamLastPulseAt < UNJAM_COOLDOWN_MS) {
    CTRL_LOG_PRINTLN(F("[UNJAM] Blocked — cooldown active"));
    return UNJAM_COOLDOWN;
  }

  // Guard: pulse limit
  if (unjamPulseCount >= UNJAM_MAX_PULSES) {
    CTRL_LOG_PRINTLN(F("[UNJAM] Blocked — max pulses reached, wait for reset"));
    return UNJAM_LIMIT;
  }
  ...
  CTRL_LOG_PRINTF("[UNJAM] Pulse %u/%u fired (300ms)\n",
...
void resetUnjamCounter() {
  if (unjamPulseCount > 0) {
    CTRL_LOG_PRINTLN(F("[UNJAM] Pulse counter reset"));
  }
  unjamPulseCount = 0;
  unjamLastPulseAt = 0;
  unjamDeEnergiseAt = 0;
