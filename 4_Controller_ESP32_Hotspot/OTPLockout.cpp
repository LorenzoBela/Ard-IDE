...
void initOtpLockoutPersistence() {
  ...
  if (!otpPrefsReady) {
    CTRL_LOG_PRINTLN(F("[EC-04] OTP lockout NVS unavailable"));
    return;
  }
  ...
  CTRL_LOG_PRINTF("[EC-04] Lockout restore: attempts=%d locked=%d\n",
                attemptCount, lockedOut ? 1 : 0);
}

void resetOtpAttempts() {
  if (attemptCount > 0) {
    CTRL_LOG_PRINTLN(F("[EC-04] OTP attempts reset"));
  }
  attemptCount   = 0;
  lockedOut      = false;
  lockoutStartAt = 0;
...
void recordFailedAttempt(unsigned long now) {
  attemptCount++;
  lastAttemptAt = now;
  CTRL_LOG_PRINTF("[EC-04] Failed attempt %d/%d\n", attemptCount, MAX_OTP_ATTEMPTS);

  if (attemptCount >= MAX_OTP_ATTEMPTS) {
    lockedOut      = true;
    lockoutStartAt = now;
    CTRL_LOG_PRINTLN(F("[EC-04] MAX ATTEMPTS — LOCKOUT ACTIVE"));
  }
...
bool isLockedOut(unsigned long now) {
  ...
  if (now - lockoutStartAt >= OTP_LOCKOUT_MS) {
    ...
    lastAttemptAt = 0;
    persistOtpState();
    CTRL_LOG_PRINTLN(F("[EC-04] Lockout expired"));
    return false;
  }
  return true;
}
