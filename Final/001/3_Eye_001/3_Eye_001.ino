...
// Final BOX_001 build: hard-pin this ESP32-CAM as OV3660_CAM_001.
...
static void initCameraIdentity() {
  snprintf(DEVICE_ID, sizeof(DEVICE_ID), "OV3660_CAM_%03u",
           (unsigned int)CAMERA_BOX_NUM);
  snprintf(EXPECTED_HARDWARE_ID, sizeof(EXPECTED_HARDWARE_ID), "BOX_%03u",
           (unsigned int)CAMERA_BOX_NUM);
  Serial.printf("[IDENTITY] Camera %s paired with %s\n",
                DEVICE_ID, EXPECTED_HARDWARE_ID);
}
    ...
    if (size > 0) {
      ...
      if (strncmp(packet, PROXY_DISCOVERY_REPLY,
        ...
        int replyPort = secondColon > firstColon
            ...
            : PROXY_PORT;
        String advertisedHardwareId =
            secondColon >= 0 ? payload.substring(secondColon + 1) : "";
        advertisedHardwareId.trim();

        if (advertisedHardwareId != String(EXPECTED_HARDWARE_ID)) {
          Serial.printf("[DISCOVERY] Ignoring proxy for %s; expected %s (%s)\n",
                        advertisedHardwareId.length() ? advertisedHardwareId.c_str() : "UNKNOWN",
                        EXPECTED_HARDWARE_ID, packet);
          continue;
        }
...
static bool proxyIdentityMatches(const String &body) {
  String trimmed = body;
  trimmed.trim();
  return trimmed == String(EXPECTED_HARDWARE_ID);
}
...
static bool pingProxyAt(const IPAddress &ip, unsigned long timeoutMs = 200) {
  ...
  client.stop();
  bool ok = httpOk && proxyIdentityMatches(body);
  if (!ok && body.length() > 0) {
    Serial.printf("[DISCOVERY] Probe ignored proxy '%s'; expected %s\n",
                  body.c_str(), EXPECTED_HARDWARE_ID);
  }
  return ok;
}
...
bool uploadToSupabase(const uint8_t *data, size_t len,
  ...
  http.addHeader("X-Camera-Id", DEVICE_ID);
