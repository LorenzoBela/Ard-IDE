...
void handleCameraClient() {
  ...
  if (strcmp(method, "GET") == 0 && reqPathStr.startsWith("/refresh-context")) {
    ...
    if (ok) {
      ...
      if (modemHttpBusy) {
        controllerContextRefreshQueued = true;
        if (geoRefreshRequested) geoStatus = "OK:BUSY";
        Serial.println("[CONTEXT] Keypad refresh requested while modem busy - queued");
      } else {
        bool needContextFields =
            !geoRefreshRequested ||
            !deliveryIdCacheValid ||
            cachedDeliveryId[0] == '\0' ||
            !otpCacheValid ||
            (!destCoordsValid && !pickupCoordsValid);
        Serial.println(needContextFields
                           ? "[CONTEXT] Keypad refresh requested - exact refresh now"
                           : "[CONTEXT] Keypad geo refresh requested - using cached context");
        bool fieldsOk = needContextFields ? refreshAuthoritativeDeliveryFieldsFromFirebase()
                                          : true;
        if (needContextFields && fieldsOk &&
            deliveryIdCacheValid && cachedDeliveryId[0] != '\0') {
          activeAfterRefresh = verifyActiveDeliveryContext("manual_refresh", true);
        }
        if (geoRefreshRequested && deliveryIdCacheValid && cachedDeliveryId[0] != '\0' &&
            (!destCoordsValid || !pickupCoordsValid)) {
          refreshTargetCoordsFromDelivery(cachedDeliveryId, true);
        }
        if (geoRefreshRequested) {
          geoStatus = refreshGeofenceForController(needContextFields ||
                                                   !destCoordsValid ||
                                                   !pickupCoordsValid);
        }
  ...
  if (strcmp(method, "GET") == 0 && reqPathStr.startsWith("/otp")) {
    ...
    if (!haveTargets && activeContextForController && !modemHttpBusy) {
      ...
      haveTargets = destCoordsValid || pickupCoordsValid;
      if (!haveTargets) {
        haveTargets = refreshTargetCoordsFromDelivery(cachedDeliveryId, true);
      }
      ...
      Serial.println("[AP] coords_fetch_busy: target refresh deferred");
    }
...
void loop() {
  ...
  // Tamper-clear is a rare admin action; check at its own slower cadence
  // to avoid burning an HTTP session on every context read cycle.
